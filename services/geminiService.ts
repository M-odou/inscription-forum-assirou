...
export const generateWelcomeMessage = async (salutation: string, name: string, company: string): Promise<string> => {
  try {
    ...
    const response = await ai.models.generateContent({
      model: MODEL_NAME,
      contents: `Tu es le Directeur de la Communication d'Assirou Sécurité. Rédige un message de bienvenue institutionnel, court (max 2-3 phrases) et très prestigieux pour ${salutation} ${name} de l'organisation ${company}, qui vient de s'inscrire au Forum Assirou Sécurité 2026. Le ton doit être solennel, professionnel et chaleureux.`,
    });
...
const getDefaultFallbackMessage = (salutation: string, name: string): string => {
  ...
  return `${label} ${name}, Assirou Sécurité est honorée de vous compter parmi les participants de notre forum 2026. Nous nous réjouissons de votre présence pour échanger sur les enjeux critiques de la sécurité globale.`;
};
